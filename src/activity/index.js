...
const performanceBlocks = [
  ...
  ></div>`,
  html`<div
    class="activitychart__activityrow__activityblock activitychart__activityrow__activityblock--mid"
  ></div>`,
  html`<div
    class="activitychart__activityrow__activityblock activitychart__activityrow__activityblock--max"
  ></div>`,
  html`<div
    class="activitychart__activityrow__activityblock activitychart__activityrow__activityblock--extra"
  ></div>`,
];

const activitybar = (hourlyArray, getInterval) => html` <div
  class="activitychart__activityrow"
>
...
const squashDataInTwoHourBlocks = (dataMatrix) => {
  return dataMatrix.map((dailyData) => {
    const res = [];
    for (let i = 1; i < Math.floor(dailyData.length / 2) * 2; i += 2) {
      //Math.floor(dailyData.length/2)*2 gets the closest even number smaller than dailyData.length
...
const renderHorizontalWeeklyBar = (data) => {
  ...
  return html`<div class="activitychart activitychart--horizontal">
    ${twoHourBlocks.map((data) => activitybar(data, intervals))}
  </div>`;
};
...
const renderVerticalWeeklyBar = (data) => {
  ...
  return html`<div class="activitychart activitychart--vertical">
    ${vertical.map((data) => activitybar(data, intervals))}
  </div>`;
};
...
export const screenTemplate = (data) => {
  return html`${[
    ...
    html`<div class="scale__wrapper">
      <div class="scale">
        ${scale(getIntervalBlocks(dayNumber.map((day) => data.data[day])))}
      </div>
    </div>`,
  ]}`;
};

const hoursScaleDash = (hoursScale, orientation) => html`<div
  class="scale__scaleblock scale__scaleblock--${orientation}"
>
  <div class="scale__scaleblock__dash"></div>
  <p class="scale__scaleblock__desc scale__scaleblock__desc--dash">
    ${hoursScale}
  </p>
</div>`;
...
const scales = (ranges) => html`
  <div class="scale__scaleblock">
    <div
      class="scale__scaleblock__colorblock scale__scaleblock__colorblock--min"
    ></div>
    <p class="scale__scaleblock__desc">${ranges[0]}</p>
  </div>
  <div class="scale__scaleblock">
    <div
      class="scale__scaleblock__colorblock scale__scaleblock__colorblock--mid"
    ></div>
    <p class="scale__scaleblock__desc">
      ${ranges[1] + " — " + (ranges[2] - 1)}
    </p>
  </div>
  <div class="scale__scaleblock">
    <div
      class="scale__scaleblock__colorblock scale__scaleblock__colorblock--max"
    ></div>
    <p class="scale__scaleblock__desc">
      ${ranges[2] + " — " + (ranges[3] - 1)}
    </p>
  ...
  <div class="scale__scaleblock">
    <div
      class="scale__scaleblock__colorblock scale__scaleblock__colorblock--extra"
    ></div>
...
const scale = (ranges) => {
  return [
    hoursScaleDash("1 час", "vertical"),
    hoursScaleDash("2 часа", "horizontal"),
    scales(ranges),
  ];
};
