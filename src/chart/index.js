...
const chartbar = ({ title, value, active }, ratio) => html`<div
  class="chart__chart__chartbar"
>
  <p
    class="chart__chart__chartbar__value${active
      ? " chart__chart__chartbar__value--active"
      : ""}"
  >
    ${value === 0 ? "" : value}
  </p>
  <div
    class="chart__chart__chartbar__bar${active
      ? " chart__chart__chartbar__bar--active"
      : ""}"
    style="height: max(8px, ${ratio * 70}%)"
  ></div>
  <div class="chart__chart__chartbar__title">${title}</div>
</div>`;
...
const chartbars = (values) => {
  ...
  return html`<div class="chart__chart">
    ${relevantValues.map((value) => chartbar(value, value.value / maxValue))}
  </div>`;
};
const chartleaders = (users) => html`<div class="chart__leaders">
  <div class="chart__leaders__leader">
    <img
      src="/images/1x/${users[0].avatar}"
      alt=""
      class="chart__leaders__leader__img"
    />
    <div class="chart__leaders__leader__meta">
      <div class="chart__leaders__leader__meta__name">${users[0].name}</div>
      <div class="chart__leaders__leader__meta__text">
        ${users[0].valueText}
      </div>
    </div>
  </div>
...
export const screenTemplate = (data) =>
  html`${[
    ...
    html`<div class="chart">
      ${[chartbars(data.values), chartleaders(data.users)]}
    </div>`,
  ]}`;
