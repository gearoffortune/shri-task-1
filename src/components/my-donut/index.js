...
export const donut = (
  ...
  const degs = [extraShare, maxShare, midShare, minShare].map(
    (x) => x * Math.PI * 2
  );
  ...
  const getCoordinates = (angleOne, angleTwo) => {
    ...
    if ((Math.PI * 5) / 4 < angleTwo && angleTwo < (Math.PI * 7) / 4) {
      return (
        res +
        `, 0% calc(50% *(1 - (${-Math.tan(Math.PI / 2 - angleTwo)}))) , 50% 50%`
      );
    }
  ...
  return html`<div class="${parentBlockName}__mydonut mydonut">
    <div
      class="${parentBlockName}__mydonut__slice mydonut__slice mydonut__slice__first"
      style="clip-path: polygon(${getCoordinates(-degs[0] / 2, degs[0] / 2)});"
    ></div>
    <div
      class="${parentBlockName}__mydonut__slice mydonut__slice mydonut__slice__second"
      style="clip-path: polygon(${getCoordinates(
        degs[0] / 2,
        degs[0] / 2 + degs[1]
      )});"
    ></div>
    <div
      class="${parentBlockName}__mydonut__slice mydonut__slice mydonut__slice__third"
      style="clip-path: polygon(${getCoordinates(
        degs[0] / 2 + degs[1],
        degs[0] / 2 + degs[1] + degs[2]
      )});"
    ></div>
    <div
      class="${parentBlockName}__mydonut__slice mydonut__slice mydonut__slice__fourth"
      style="clip-path: polygon(${getCoordinates(
        degs[0] / 2 + degs[1] + degs[2],
        degs[0] / 2 + degs[1] + degs[2] + degs[3]
      )});"
    ></div>
    <div class="${parentBlockName}__mydonut__label mydonut__label">
      <div
        class="${parentBlockName}__mydonut__label__heading mydonut__label__heading"
      >
        ${centralTextLabel}
      </div>
